Shader "Custom/sine_wave_reflection" {
	Properties{
		...
		SubShader{
			...
			Wave createRandomWave(float ampMax, float waveLengthMax, float maxSpeed) 
			{
				float3 seed = mul(unity_ObjectToWorld, float4(0, 0, 0, 1)).xyz;
				float3 rand = random(seed);
				
				// Generate some random settings for the wave
				float amp = rand.x * ampMax;
				float waveLength = rand.y * waveLengthMax;
				float speed = rand.z * maxSpeed;
				float3 dir = rand;

				// Compute rest of wave equation
				float freq = (2 * PI) / waveLength;
				float phase = (speed * 2 * PI) / waveLength;
				
				Wave w = { freq, amp, phase, dir.xy };
				return w;
			}
			...
			void vert(inout appdata vIn)
			{
				...
				Wave waves[5] = {
					createRandomWave(_Amp, _WaveLength, _Speed),
					createRandomWave(_Amp, _WaveLength, _Speed),
					createRandomWave(_Amp, _WaveLength, _Speed),
					createRandomWave(_Amp, _WaveLength, _Speed),
					createRandomWave(_Amp, _WaveLength, _Speed)
